...
# Styling variables
SOFT_SHADOW = (
    "0 10px 25px -5px rgba(53, 79, 54, 0.15), 0 8px 10px -6px rgba(53, 79, 54, 0.1)"
)
HOVER_SHADOW = (
    "0 25px 30px -5px rgba(53, 79, 54, 0.25), 0 15px 15px -5px rgba(53, 79, 54, 0.15)"
)
...
class ClinicState(rx.State):
    ...
    def set_selected_reason(self, reason: str):
    ...
    def select_day(self, day: str):
        ...
        try:
            response = requests.get(
                f"{API_URL}/schedule?date={date_str}&doctor={self.selected_doctor}",
                timeout=3,
            )
            if response.status_code == 200:
    ...
    def submit_appointment(self):
        ...
        payload = {
            "reason_of_appointment": self.selected_reason,
            "doctor_name": self.selected_doctor,
            "appointment_at": formatted_datetime,
        }
        ...
        try:
            ...
            if response.status_code == 201:
                data = response.json()
                msg = (
                    f"Slot taken. Added to waitlist for {formatted_datetime}."
                ...
                return rx.window_alert(
                    response.json().get("detail", "Slot taken and already waitlisted.")
                )
            else:
...
def animated_container(content: rx.Component, is_visible: bool) -> rx.Component:
    return rx.box(
        ...
        transform=rx.cond(
            is_visible, "translateY(0) scale(1)", "translateY(-15px) scale(0.98)"
        ),
...
def auth_screen() -> rx.Component:
    return rx.vstack(
        ...
        rx.text(
            "General Care, Cosmetic & Emergency Dentistry",
            color="white",
            opacity="0.8",
            margin_bottom="1.5em",
            font_size="1.1em",
            ...
            rx.vstack(
                rx.cond(
                    ...
                    rx.input(
                        ...
                        value=ClinicState.register_name,
                        on_change=ClinicState.set_register_name,
                        border_radius="12px",
                        size="3",
                        bg="white",
                        width="100%",
                ...
                rx.input(
                    ...
                    value=ClinicState.phone,
                    on_change=ClinicState.set_phone,
                    border_radius="12px",
                    size="3",
                    bg="white",
                    width="100%",
                ...
                rx.input(
                    ...
                    type="password",
                    value=ClinicState.password,
                    on_change=ClinicState.set_password,
                    border_radius="12px",
                    size="3",
                    bg="white",
                    width="100%",
                ...
                rx.button(
                    rx.cond(
                        ClinicState.auth_mode == "login",
                        "Need an account? Register",
                        "Have an account? Log in",
                    ),
                    on_click=ClinicState.toggle_auth_mode,
                    bg="transparent",
                    color=TEXT_LIGHT,
                    width="100%",
                    transition="all 0.3s ease",
                    _hover={"color": PALETTE_DARK},
                ),
                width="100%",
                spacing="4",
            ),
...
def entry_card(item: dict, is_waitlist: bool) -> rx.Component:
    ...
    return rx.hstack(
        rx.vstack(
                    ...
                    rx.box(
                        rx.text(
                            rx.cond(
                                item["priority"] == 3,
                                "High",
                                rx.cond(item["priority"] == 2, "Medium", "Low"),
                            ),
                            font_size="0.75em",
                            font_weight="700",
                        ...
                        bg=rx.cond(
                            item["priority"] == 3,
                            "#fee2e2",
                            rx.cond(item["priority"] == 2, "#fef3c7", "#dcfce7"),
                        ),
                        color=rx.cond(
                            item["priority"] == 3,
                            "#991b1b",
                            rx.cond(item["priority"] == 2, "#92400e", "#166534"),
                        ),
                        padding="0.2em 0.8em",
                        border_radius="8px",
                        margin_left="1em",
            ...
            rx.text(
                item["reason_of_appointment"], color=TEXT_LIGHT, font_size="0.95em"
            ),
            rx.text(
                item["doctor_name"],
        ...
        rx.button(
            "Cancel",
            on_click=lambda: ClinicState.cancel_existing_appointment(
                item[id_key], is_waitlist
            ),
            bg="#fca5a5",
            color="#7f1d1d",
            border_radius="8px",
...
def booking_widget() -> rx.Component:
    return rx.vstack(
        rx.cond(
            ...
            rx.button(
                "Create Appointment",
                on_click=ClinicState.start_booking,
                border_radius="999px",
                padding="1.5em 3em",
                bg=PALETTE_DARK,
                color="white",
            ...
            rx.vstack(
                ...
                rx.box(
                    rx.hstack(
                        ...
                        rx.text(
                            "Emergency? For severe pain, swelling, bleeding, or trauma, please bypass this app and visit the clinic immediately.",
                            color="#7f1d1d",
                            font_size="0.9em",
                            font_weight="500",
                        ),
                ...
                rx.hstack(
                    ...
                    rx.button(
                        "Cancel",
                        on_click=ClinicState.cancel_booking,
                        bg="transparent",
                        color=TEXT_LIGHT,
                        transition="all 0.2s ease",
                        _hover={"color": TEXT_DARK, "background": INPUT_BG},
                    ),
                    width="100%",
                ),
                        ...
                        rx.grid(
                            rx.foreach(
                                ...
                                lambda doc: rx.button(
                                    ...
                                    padding="1.5em",
                                    bg=rx.cond(
                                        ClinicState.selected_doctor == doc["name"],
                                        ACTIVE_GREEN,
                ...
                animated_container(
                    rx.box(
                        ...
                        rx.vstack(
                            rx.text(
                                "Reason for visit?", font_weight="500", color=TEXT_DARK
                            ),
                            rx.input(
                                placeholder="e.g., Routine checkup, tooth pain...",
                                value=ClinicState.selected_reason,
                                on_change=ClinicState.set_selected_reason,
                                border_radius="12px",
                                size="3",
                                bg=f"rgba(184, 206, 183, 0.4)",
                                width="100%",
                                **INPUT_STYLE,
                            ),
                            rx.button(
                                "Next",
                                on_click=ClinicState.confirm_reason,
                                border_radius="12px",
                                padding="1em 2em",
                                bg=PALETTE_DARK,
                                color="white",
                    ...
                    rx.box(
                        ...
                        rx.vstack(
                            ...
                            rx.hstack(
                                rx.select(
                                    ClinicState.years,
                                    value=ClinicState.selected_year,
                                    on_change=ClinicState.set_selected_year,
                                    bg=f"rgba(184, 206, 183, 0.4)",
                                    **INPUT_STYLE,
                                ),
                                rx.select(
                                    ClinicState.months,
                                    value=ClinicState.selected_month,
                                    on_change=ClinicState.set_selected_month,
                                    bg=f"rgba(184, 206, 183, 0.4)",
                                    **INPUT_STYLE,
                                ),
                                width="100%",
                            ...
                            rx.grid(
                                rx.foreach(
                                    ClinicState.calendar_days,
                                    lambda day_dict: rx.cond(
                                        day_dict["day"] != "",
                                        rx.button(
                                            day_dict["day"],
                                            on_click=rx.cond(
                                                ~day_dict["disabled"],
                                                ClinicState.select_day(day_dict["day"]),
                                                None,
                                            ),
                ...
                animated_container(
                    rx.box(
                        ...
                        rx.vstack(
                            ...
                            rx.grid(
                                rx.foreach(ClinicState.daily_schedule, time_slot_card),
...
def dashboard() -> rx.Component:
    return rx.vstack(
        ...
        rx.vstack(
            ...
            rx.vstack(
                rx.heading(
                    f"{ClinicState.client_name}'s Upcoming Appointments",
                    size="5",
                    color=TEXT_DARK,
                    margin_bottom="0.5em",
                ),
                rx.cond(
                    ...
                    rx.foreach(
                        ClinicState.my_appointments,
                        lambda appt: entry_card(appt, is_waitlist=False),
                    ),
                    rx.text(
            ...
            rx.vstack(
                rx.heading(
                    f"{ClinicState.client_name}'s Waitlist Entries",
                    size="5",
                    color=TEXT_DARK,
                    margin_bottom="0.5em",
                ),
                rx.cond(
                    ...
                    rx.foreach(
                        ClinicState.my_waitlist,
                        lambda appt: entry_card(appt, is_waitlist=True),
                    ),
                    rx.text(
                        "You have no waitlisted appointments.",
                        color=TEXT_DARK,
                        opacity="0.8",
                        font_style="italic",
                        padding_left="0.5em",
...
def index() -> rx.Component:
    return rx.box(
        rx.cond(ClinicState.is_logged_in, dashboard(), auth_screen()),
